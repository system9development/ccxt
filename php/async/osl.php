...
class osl extends Exchange {
    ...
    public function fetch_markets($params = array ()) {
         ...
         */
        $markets = yield $this->v4PrivateGetInstrument ($params);
        // array(
        //     ...
        //     array(
        //         symbol => 'SNXUSD',
    ...
    public function fetch_balance($params = array ()) {
        ...
        $response = yield $this->v4PrivateGetUserWallet ($params);
        // array(
        //     array(
    ...
    public function fetch_order_book($symbol, $limit = null, $params = array ()) {
        ...
        $response = yield $this->v4PrivateGetOrderBookL2 (array_merge($request, $params));
        // {
        //     $symbol => 'BTCUSDT',
        //     asks => array(
        //         array( '20202', '3' ),
    ...
    public function sign($path, $api = 'public', $method = 'GET', $params = array (), $headers = null, $body = null) {
        ...
        if ($type === 'private') {
            ...
            if (($method === 'GET') || ($method === 'DELETE')) {
                ...
                $data = $method . $this->encode('/api' . '/' . $version . '/' . $path) . $this->encode($tonce);
                $signature = $this->hmac($this->encode($data), base64_decode($this->secret), 'sha512', 'base64');
                $headers['api-signature'] = $signature;
            } else {
                $body = $query;
                $headers['Content-Type'] = 'application/x-www-form-urlencoded';
                $data = $method . $this->encode($path) . $this->encode($tonce) . $this->encode(json_encode ($body));
                $signature = $this->hmac($this->encode($data), base64_decode($this->$'secret'), 'sha512', 'base64');
                $headers['api-signature'] = $signature;
            }
        } else {
