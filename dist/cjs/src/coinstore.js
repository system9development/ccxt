...
var coinstore$1 = require('./abstract/coinstore.js');
...
class coinstore extends coinstore$1 {
    ...
    parsePrivateTrade(trade, market = undefined) {
        ...
        const price = Precise["default"].stringDiv(cost, amount);
    ...
    async createOrder(symbol, type, side, amount, price = undefined, params = {}) {
        ...
        if (request['ordType'] === 'MARKET' && side.toLowerCase() === 'buy') {
            ...
            if (amount !== undefined) {
                throw new errors.BadRequest(this.id + ' createOrder () does not support the amount parameter for market buy orders, for the amount of quote currency to spend use the ordAmt parameter');
            }
            if (!ordAmt) {
                throw new errors.BadRequest(this.id + ' createOrder () requires an ordAmt parameter for market buy orders, for the amount of quote currency to spend');
            }
    ...
    parseOrder(order, market = undefined) {
        ...
        const remaining = Precise["default"].stringSub(amount, filledAmount);
        const average = filledAmountNumber !== 0 ? Precise["default"].stringDiv(cost, filledAmount) : undefined;
        return this.safeOrder({
            'info': order,
