...
var deepwaters$1 = require('./abstract/deepwaters.js');
...
//  ---------------------------------------------------------------------------
class deepwaters extends deepwaters$1 {
    describe() {
    ...
    async fetchTicker(symbol, params = {}) {
         ...
         */
        if (!symbol) {
            throw new errors.ArgumentsRequired('symbol must be provided to deepwaters#fetchTicket()');
        }
        const tickers = await this.fetchTickers([symbol]);
        if (tickers.length === 0) {
            throw new errors.BadSymbol('Symbol not found. Is it available in  deepwaters#fetchMarkets()?');
        }
    ...
    async fetchTickers(symbols = undefined, params = {}) {
        ...
        for (let i = 0; i < markets.length; i++) {
            ...
            const timestamp = this.parseNumber(Precise["default"].stringDiv(quotedAtMicros, '1000', 0));
    ...
    parseOrder(order, market = undefined) {
        ...
        this.safeString(order, 'modifiedAtMicros');
        let createdTimestampMs = undefined;
        let updatedTimestampMs = undefined;
        if (createdTimestampMicros) {
            createdTimestampMs = Precise["default"].stringDiv(createdTimestampMicros, '1000', 0);
        }
        let timeInForce = 'GTC';
    ...
    async fetchBalance(params = {}) {
        ...
        const timestamp = this.parseNumber(Precise["default"].stringDiv(modifiedAtMicros, '1000', 0));
        const datetime = this.iso8601(timestamp);
        const output = {
            'timestamp': timestamp,
            'datetime': datetime,
        ...
        for (let i = 0; i < balances.length; i++) {
            ...
            if (serviceName === 'accounting.available') {
                ...
                const used = Precise["default"].stringAdd(output[currencyCode]['used'], amount);
        ...
        for (let i = 0; i < keys.length; i++) {
            const key = keys[i];
            const total = Precise["default"].stringAdd(output['used'][key], output['free'][key]);
    ...
    async fetchOrderBook(symbol, limit = undefined, params = {}) {
        ...
        const timestamp = this.parseNumber(Precise["default"].stringDiv(timestampMicros, '1000', 0));
    ...
    async fetchOrders(symbol = undefined, since = undefined, limit = undefined, params = {}) {
        ...
        if (since !== undefined) {
            since = Precise["default"].stringMul(this.decimalToPrecision(since, number.TRUNCATE, this.markets[symbol]['precision']['amount']), '1000');
            request['created-at-or-after-micros'] = since;
        }
        if (limit !== undefined) {
            limit = this.decimalToPrecision(limit, number.TRUNCATE, this.markets[symbol]['precision']['amount']);
            request['limit'] = limit;
        }
    ...
    async fetchMyTrades(symbol = undefined, since = undefined, limit = undefined, params = {}) {
        ...
        for (let i = 0; i < trades.length; i++) {
            ...
            const timestamp = this.parseNumber(Precise["default"].stringDiv(createdAtMicros, '1000', 0));
            const datetime = this.iso8601(timestamp);
            ...
            const cost = Precise["default"].stringMul(this.safeString(trade, 'price'), this.safeString(trade, 'quantity'));
    ...
    async createOrder(symbol, type, side, amount, price = undefined, params = {}) {
        ...
        const timestamp = this.parseNumber(Precise["default"].stringDiv(respondedAtMicros, '1000', 0));
        const datetime = this.iso8601(timestamp);
    ...
    async fetchOrder(id, symbol = undefined, params = {}) {
        ...
        if (typeof id !== 'string') {
            throw new errors.ArgumentsRequired(this.id + ' cancelOrder () requires a string id');
        }
    ...
    sign(path, api = 'public', method = 'GET', params = {}, headers = undefined, body = undefined) {
        ...
        */
        ...
        if (api === 'private') {
            ...
            // calculate signature
            const signature = this.signHash(this.hash(message, sha3.keccak_256), this.secret);
    ...
    handleError(response = {}) {
        ...
        throw new errors.ExchangeError(code + ': ' + error + ' ' + status);
    }
