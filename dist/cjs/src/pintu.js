...
var pintu$1 = require('./abstract/pintu.js');
...
class pintu extends pintu$1 {
        ...
        return this.deepExtend(super.describe(), {
            ...
            'exceptions': {
                'exact': {
                    ...
                    '7': errors.OrderNotFound,
                    '8': errors.ExchangeError,
                    '11': errors.InsufficientFunds,
                    '12': errors.InvalidNonce,
                },
                'broad': {
                    // code 2, http 400:
                    'page size': errors.BadRequest,
                    'order id': errors.InvalidOrder,
                    'should be greater': errors.BadRequest,
                    'within 24': errors.BadRequest,
                    'is required': errors.ArgumentsRequired,
                    'must be greater than 0': errors.BadRequest,
                    'must be greater than zero': errors.BadRequest,
                    'must be greater than minimum': errors.BadRequest,
                    'duplicate request id': errors.DuplicateOrderId,
                    'invalid asset': errors.BadSymbol,
                    // code 3, http 200:
                    'exceeded maximum order size': errors.InvalidOrder,
                    'insufficient': errors.InvalidOrder,
                    'should be specified': errors.ArgumentsRequired,
                    'invalid price': errors.InvalidOrder,
                    'too many decimals': errors.InvalidOrder,
                    'far away limit price': errors.InvalidOrder,
                    'exceeded trading daily limit': errors.PermissionDenied,
                    'max number of open orders.': errors.PermissionDenied, // Max number of open orders.
                },
            },
            'commonCurrencies': {},
        });
    ...
    async createOrder(symbol, type, side, amount, price = undefined, params = {}) {
        ...
        */
        ...
        if (type === 'market' && side === 'buy') {
            ...
            if (amount !== undefined) {
                throw new errors.BadRequest(this.id + ' createOrder () does not support the amount parameter for market buy orders, for the amount of quote currency to spend use the notional parameter');
            }
            if (!notional || Number.isNaN(notional)) {
                throw new errors.BadRequest(this.id + ' createOrder () requires a notional parameter for market buy orders, for the amount of quote currency to spend');
            }
        ...
        */
        ...
        if (since !== undefined) {
            ...
            if (!until || Number.isNaN(difference) || difference <= 0 || difference > maxDifference) {
                throw new errors.BadRequest(this.id + ' fetchOrders() requires a t_end parameter to be specified with since, and must be max 24 hours from since');
            }
    ...
    parseOrder(order, market) {
        ...
        const orderResponse = {
            ...
            'remaining': this.parseNumber(Precise["default"].stringSub(amount.toString(), filledAmount)),
            'cost': cost,
            'status': status,
        };
    ...
    async fetchMyTrades(symbol, since = undefined, limit = undefined, params = {}) {
        ...
        */
        ...
        if (since !== undefined) {
            ...
            const maxDifference = this.parseTimeframe('1d') * 1000;
            if (!until || Number.isNaN(difference) || difference <= 0 || difference > maxDifference) {
                throw new errors.BadRequest(this.id + ' fetchMyTrades() requires a t_end parameter to be specified with since, and must be max 24 hours from since');
            }
            request['t_start'] = since;
            request['t_end'] = until;
        }
    ...
    parseTrade(trade, market) {
        ...
        const cost = this.parseNumber(Precise["default"].stringMul(amount.toString(), price.toString()));
        let rate = Precise["default"].stringDiv(fee.toString(), amount.toString());
        if (feeCurrency === market['quote']) {
            rate = Precise["default"].stringDiv(fee.toString(), cost.toString());
        }
    ...
    async fetchOrder(id, symbol, params = {}) {
        ...
        */
        ...
        if (!id && !clientOrderId) {
            throw new errors.BadRequest(this.id + ' fetchOrder() requires either an order id or params.client_order_id');
        }
        await this.loadMarkets();
        const request = {
            'symbol': symbolId,
    ...
    sign(path, api = 'public', method = 'GET', params = {}, headers = {}, body = {}) {
        ...
        */
        ...
        const signature = this.hmac(this.encode(signString), this.encode(this.secret), sha256.sha256, 'hex');
        body = {
            'api_key': this.apiKey,
            'method': path,
            'params': params,
    ...
    handleErrors(code, reason, url, method, headers, body, response, requestHeaders, requestBody) {
        ...
        if (responseCode !== '0') {
            ...
            throw new errors.BadRequest(this.id + ' failed with code ' + responseCode + ' and message ' + message + '. Check your arguments and parameters');
        }
    }
}
