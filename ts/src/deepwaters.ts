...
export default class deepwaters extends Exchange {
    ...
    async fetchOrders (symbol = undefined, since = undefined, limit = undefined, params = {}) {
        ...
        if (since !== undefined) {
            since = Precise.stringMul (this.decimalToPrecision (since, TRUNCATE, this.markets[symbol]['precision']['amount']), '1000');
            request['created-at-or-after-micros'] = since;
        }
        if (limit !== undefined) {
            limit = this.decimalToPrecision (limit, TRUNCATE, this.markets[symbol]['precision']['amount']);
            request['limit'] = limit;
        }
